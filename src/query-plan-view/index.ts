 ...
 * as plain text into an editor (which `firebird.explainPlan` still does, unchanged). Phase 2 of
 * `docs/roadmap/query-plan-visualizer.md` — estimated plan only, no actual-plan monitoring
 * overlay, table/source views, saved-plan import, or Copilot analysis yet (see that doc for
...
export class QueryPlanView extends QueryResultsView implements vscode.Disposable {
  ...
  private async fetchAndSend(): Promise<void> {
    ...
    this.send({ command: "planData", data: { blocks, raw: planText } });
  }
}
